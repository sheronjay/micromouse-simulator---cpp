...
int walls[maze_size][maze_size][4]; // [row][column][N,E,S,W][times visited]
...
int get_next_move2(){
    int best_direction = -1;
    int lowest_visit_count = INT_MAX;
    ...
    for(int d = 0; d < 4; d++){
        ...
        if(next_row >= 0 && next_row < maze_size && 
           ...
           !walls[position[0]][position[1]][d]){
            
            // Check if this cell has been visited fewer times
            if(walls[next_row][next_col][4] < lowest_visit_count){
                lowest_visit_count = walls[next_row][next_col][4];
                best_direction = d;
            }
        }
    }

    return best_direction; // Returns -1 if no valid moves found
}
